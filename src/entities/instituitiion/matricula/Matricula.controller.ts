...
import { MatriculaCreateData } from './Matricula';
...
export class MatriculaController {
  async create(request: Request, response: Response) {
    const data: MatriculaCreateData = request.body;
  ...
  async update(request: Request, response: Response) {
    ...
    const data: Partial<MatriculaCreateData> = request.body;

    if (!validate(id)) {
      response.status(401).json('Bad request');
    }

    const country = await prisma.matricula
      .findUnique({
        where: {
          id,
        },
  ...
  async delete(request: Request, response: Response) {
    ...
    const country = await prisma.matricula.findUnique({
      where: {
        id,
      },
    });
